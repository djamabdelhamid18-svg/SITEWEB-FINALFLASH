...
export const DEFAULT_SEED_PRODUCTS = [
  {
    id: 1,
    title: "Baggy Jogger",
    category: "pants",
    ...
    conditionDetails:
      "قطعة حصرية مصممة من تنفيذ Finalflash. تطريز Flaming Dice دقيق ومكثف، خياطة ثلاثية متينة، ومطاط خصر مريح مع رباط قابل للتعديل.",
    ...
    description:
      "بنطال جوغر بقصة باغية فضفاضة ومريحة. قماش قطني سميك فائق التحمل، وتطريز Flaming Dice المميز من تنفيذ Finalflash.",
  ...
  },
  {
    ...
    conditionDetails:
      "قطعة ثريفت أصلية واردة بحالة ممتازة وخالية تماماً من العيوب، الدرزات سليمة 100%، وتم غسلها وتعقيمها بالبخار قبل العرض.",
    ...
    description:
      "تيشيرت Gymshark أسود أصلي بطباعة بيضاء نظيفة. ملمس ناعم وخفيف، ملائم لارتداء يومي ستريت وير أو رياضي.",
  ...
  },
  {
    ...
    conditionDetails:
      "قطعة فينتاج نادرة بلون أزرق بحري أصلي، الرسمة الكلاسيكية مطبوعة بجودة عالية دون تشققات ظاهرة، وياقة متماسكة خالية من التمدد.",
    ...
    description:
      "تيشيرت Hard Rock Cafe كلاسيكي بتصميم فينتاج نادر. خامة قطنية ممتازة وقصة مربعة ستريت وير أصيلة لا تتكرر.",
  ...
  },
  {
    ...
    conditionDetails:
      "طقم متناسق يتضمن بنطال Baggy Jogger مع تيشيرت ستريت وير أصلي (Gymshark أو Hard Rock). توفير فوري بقيمة 600 دج.",
    ...
    description:
      "اقتنِ اللوك الكامل بخصم خاص: بنطال Baggy Jogger مطرز مع تيشيرت ستريت وير أصلي من اختيارك بسعر 4,300 دج بدل 4,900 دج.",
  ...
  },
  {
    ...
    conditionDetails:
      "بنطال Carhartt أصلي بلون بني ترابي كلاسيكي، قماش متين جداً بحالة نظيفة وممتازة، خالي من أي تمزقات، وسحاب نحاسي أصلي.",
    ...
    description:
      "قطعة أرشيفية نادرة من كارهارت. قماش كانفاس بني فائق المتانة، جيوب وورك وير تقليدية، وقصة باغية تجسد ثقافة الستريت وير الكلاسيكية.",
  ...
  },
  {
    ...
    conditionDetails:
      "حذاء كونفيرس تشاك تايلور برقبة عالية بحالة ممتازة، الكانفاس متماسك والنعل المطاطي سليم ومرن، خالي من أي تلف أو اهتراء جانبي.",
    ...
    description:
      "حذاء Converse Chuck Taylor All Star High كلاسيكي بلون رمادي دخاني. حذاء ستريت وير أيقوني يعبر عن ثقافة الشارع الحقيقية.",
    images: [
      "Converse-1.jpg",
      "Converse-2.jpg",
      "Converse-3.jpg",
...
export async function ensureProductsSeeded(): Promise<void> {
  if (isSeeded) return;
  try {
    const countResult = await db.execute(sql`SELECT count(*)::int as count FROM products`);
    ...
    if (count === 0) {
      ...
      // Reset sequence so auto-increment IDs for new products start after 6
      await db.execute(sql`SELECT setval('products_id_seq', (SELECT MAX(id) FROM products))`);
    }
    isSeeded = true;
  } catch (err) {
    // Database might be uninitialized during offline development or test runs
  }
}
